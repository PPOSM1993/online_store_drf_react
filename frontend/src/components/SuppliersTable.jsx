...
const SuppliersTable = () => {
    ...
    const fetchSuppliers = async (q = "") => {
        ...
        const res = await axios.get(`http://localhost:8000/api/suppliers/search/?q=${q}`, {
            headers: {
    ...
    const handleDelete = async (id) => {
        const confirm = await Swal.fire({
            title: "¿Estás seguro?",
            text: "Este proveedor será eliminado permanentemente.",
            icon: "warning",
        ...
        if (confirm.isConfirmed) {
            try {
                const token = localStorage.getItem("access_token");
                await axios.delete(`http://localhost:8000/api/suppliers/delete/${id}/`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
